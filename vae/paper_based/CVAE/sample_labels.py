...
def _default_prop_names(num_prop: int) -> list[str]:
    """Best-effort human-readable names for conditioning columns.

    Notes:
      - This repo often uses two properties: MW, LogP.
      - If training used a different set/order, you can still interpret
        columns by index.
    """
    if int(num_prop) == 2:
        return ['MW', 'LogP']
...
def _compute_rdkit_descriptors(mols: list[Chem.Mol]) -> dict[str, list[float]]:
    """Compute a small set of RDKit descriptors for already-validated molecules.

    This is meant as a cheap/fast baseline (PoC) to compare to predicted labels.
    For many properties (bioactivity, docking, etc.), RDKit won't be available,
    and the learned `pred_*` columns become the main signal.
    """
    mw: list[float] = []
    logp: list[float] = []
    tpsa: list[float] = []
...
def _model_supports_label_prediction(model) -> bool:
    """Return True if this model instance can predict labels from latent z."""
...
def _predict_labels_from_latent(*, model, latent_vector: np.ndarray) -> Optional[np.ndarray]:
    ...
    Notes:
      - Predictions are in the same scale as training targets (often normalized).
      - Returns None if the loaded checkpoint/model has no label head.
    """
...
def _sample_batch_strings(
    *,
    ...
    Returns:
      raw_strings: decoded strings (still containing 'E' padding)
      latent_vector: sampled latent vectors used to generate the batch
      pred_labels: optional predicted labels aligned with raw_strings (or None)
    """
...
) -> tuple[list[tuple[str, Chem.Mol, Optional[np.ndarray]]], dict]:
    """Like utils.collect_new_unique_from_raw(), but also carries an aligned payload.

    This is used to keep (canonical_smiles, mol) aligned with predicted labels.
    """
    accepted: list[tuple[str, Chem.Mol, Optional[np.ndarray]]] = []
    stats = {
        'total_generated': 0,
        'accepted': 0,
        'invalid_or_empty': 0,
...
def converts_sweep_to_list(df:pd.DataFrame) -> tuple[list[Chem.Mol], list[str], Optional[list[np.ndarray]]]:
    all_mols = []
    all_smiles = []
    all_pred = []
    have_pred = False
...
def converts_sweep_to_list_with_targets(
    ...
    *,
    prop_names: list[str],
) -> tuple[list[Chem.Mol], list[str], Optional[list[np.ndarray]], Optional[list[tuple[float, ...]]]]:
    """Like converts_sweep_to_list(), but preserves sweep target properties per molecule.

    Keeping this alignment makes it much easier to analyze:
      target -> generated SMILES -> RDKit descriptors -> predicted labels.
    """
    ...
    for _, row in df.iterrows():
        ...
        # Defensive checks: keep alignment exact.
        ...
        if preds is not None:
            ...
            # Keep list lengths aligned when some sweep points have no preds.
            all_pred.extend([None for _ in range(len(smiles))])

    if not have_pred:
        return all_mols, all_smiles, None, all_targets

    # Filter None placeholders (shouldn't happen if model supports labels), but keep safety.
