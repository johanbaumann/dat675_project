    ...
    *,
    ...
    for fold in iterations:
        ...
        if quality_path and os.path.isfile(quality_path):
            ...
            row_total_generated = coerce_int(row.get('total_generated'))
            row_accepted = coerce_int(row.get('accepted'))
            row_invalid = coerce_int(row.get('invalid_or_empty'))
            row_discarded = coerce_int(row.get('discarded_cleanup'))
            row_in_training = coerce_int(row.get('in_training'))
            row_duplicate = coerce_int(row.get('duplicate'))
            row_rejected = coerce_int(row.get('rejected_by_filter'))
            row_salt = coerce_int(row.get('salt_stripped'))
            row_tautomer = coerce_int(row.get('tautomer_canonicalized'))
        ...
        if analysis_summary_path and os.path.isfile(analysis_summary_path):
            ...
            div_internal = coerce_float(summary.get('diversity_internal_score'))
            mean_sim_internal = coerce_float(summary.get('mean_tanimoto_internal_all_pairs'))
            internal_mode = summary.get('internal_similarity_mode')
            internal_pairs_used = coerce_int(summary.get('internal_similarity_pairs_used'), default=0)
            internal_pairs_total = coerce_int(summary.get('internal_similarity_pairs_total'), default=0)
            n_rows = coerce_int(summary.get('num_generated_rows'), default=generated_rows)
            pred_mae = coerce_float(summary.get('pred_vs_ref_mae'))
            pred_std_ae = coerce_float(summary.get('pred_vs_ref_std_ae'))
            pred_count = coerce_int(summary.get('pred_vs_ref_count'), default=0)
            target_mae = coerce_float(summary.get('target_vs_ref_mae'))
            target_std_ae = coerce_float(summary.get('target_vs_ref_std_ae'))
            target_count = coerce_int(summary.get('target_vs_ref_count'), default=0)
...
def _write_cv_combo_error_summary_csv(*, cross_fold_summary_path: str, artifacts_root: str) -> Optional[str]:
    ...
    for row in per_fold:
        pred_count = coerce_int(row.get('pred_vs_ref_count'), default=0)
        pred_mae = coerce_float(row.get('pred_vs_ref_mae'))
        pred_std = coerce_float(row.get('pred_vs_ref_std_ae'))
        target_count = coerce_int(row.get('target_vs_ref_count'), default=0)
        target_mae = coerce_float(row.get('target_vs_ref_mae'))
        target_std = coerce_float(row.get('target_vs_ref_std_ae'))
...
def _write_cv_combo_metric_plots(*, cross_fold_summary_path: str, artifacts_root: str) -> dict:
    ...
    for ax, (metric_key, metric_title, color) in zip(axes_list, metric_specs):
        values = np.asarray([coerce_float(row.get(metric_key)) for row in per_fold], dtype=float)
    ...
    for ax2, (metric_key, metric_title, color) in zip(axes2_list, metric_specs):
        metric_vals = np.asarray([coerce_float(row.get(metric_key)) for row in per_fold], dtype=float)
...
def _build_train_config_for_iteration(
    ...
    *,
    ...
    return deep_update_dict(base_train_config, override)
...
def main() -> None:
    ...
    train_script = _resolve_script_path(
        cfg.get('train', {}).get('script', 'scripts/train_labels.py'),
    ...
    for pair in fold_pairs:
        ...
        if sampling_enabled:
            ...
            target_sampling_mode = resolve_target_sampling_mode(fold_sampling_cfg)
