# Conditional $\beta$-VAE for molecular generation (PyTorch)

This project is loosley based on the code of: [CVAE](https://github.com/jaechanglim/CVAE)
...
Data should be placed in a folder parallel to the `vae/` project directory:
...
│   └── README.md
├── vae/
│   ├── run_fold_pipeline.py
│   ├── fold_pipeline_config.example.yaml
│   └── ...
```

### CSV Format Requirements

Each fold CSV file should have:

- **`smiles`** column: SMILES strings (canonical or non-canonical)
- **`pIC50`** column (or your configured label): Property values as floats
- Optional: Additional property columns (will be ignored unless configured)

Example:

```csv
smiles,pIC50
CC(=O)NC(Cc1ccccc1)C(O)CNC1CC2(CCC2)Oc2ncc(CC(C)C)cc21,8.3010302
CC(=O)NC(Cc1ccccc1)C(O)CNC1CC2(CCC2)Oc2ncc(Cl)cc21,6.0190883
```

### Fold File Naming

Fold files are discovered using the `fold_glob` pattern in the config (default: `*fold_*.csv`).

- Files must end with an integer index: `fold_0.csv`, `original_fold_0.csv`, etc.
- Names like `fold_0.csv`, `cv_fold_0.csv`, or `original_fold_0.csv` all match
- Place all folds in the same directory specified by `train_validation_folds_dir`

### Optional: Heldout Test Set

If using `exclude_heldout_scaffolds: true` in sampling config, provide:
...
The heldout CSV should have at least a `smiles` column (or column name configured in `smiles_column`).
...
- Data in `../data/combination_1300_molecules_and_0_%_synthetic/` with fold CSVs
- Conda environment set up

**Run on a single fold to test:**
...
1. Train the β-CVAE on fold 0 training data (saves to `save/fold_pipeline_runs/cv_iteration_0/`)
2. Generate molecules from the trained model (saves to `fold_pipeline_outputs/cv_iteration_0/generated/`)
3. Analyze the generated molecules (saves plots to `fold_pipeline_outputs/cv_iteration_0/analysis/`)

Expected runtime: **10-30 minutes** on GPU (depends on batch_size, num_epochs, dataset size).

**Check the output:**

```bash
# Model checkpoint and config
cat save/fold_pipeline_runs/cv_iteration_0/training/training_config.json

# Generated molecules
head fold_pipeline_outputs/cv_iteration_0/generated/generated.csv

# Analysis summary
cat fold_pipeline_outputs/cv_iteration_0/analysis/analysis_summary.json
```
...
**`fold_pipeline_config.example.yaml` is the single point of control for the entire pipeline.** All settings for training, sampling, analysis, and data handling flow through this one configuration file. There are no additional command-line flags for model hyperparameters, sampling strategies, or analysis settings—everything is configured in the YAML file.

### Key Points:

- **Single source of truth**: Modify the config file to control every aspect of the pipeline
  - Data paths (`train_validation_folds_dir`, `heldout_smiles_csv`)
  - Training parameters (`model.mode`, `latent_size`, `learning_rate`, `batch_size`, `num_epochs`)
  - Sampling strategy (`target_sampling_mode`, `num_unique`, `temperature`)
  - Analysis settings (`analysis.profile`, `analysis.overrides`)
  - Cleanup and output management

- **Command-line arguments are minimal**: Only `--config` and `--only-fold` are available for CLI control
  - `--config`: Specify which YAML file to use (default: `fold_pipeline_config.example.yaml`)
  - `--only-fold`: Restrict execution to a single fold for debugging

- **Multiple experiments**: Copy and modify the config file for different experimental setups (see [Using Custom Configuration Files](#using-custom-configuration-files))

- **Reproducibility**: Save your config file along with model outputs. The training stage also auto-saves `training_config.json` containing all hyperparameters for that run
...
If you want to **disable training** and only **run analysis**, modify the config:

```yaml
train:
  enabled: false      # <- Skip training
sampling:
  enabled: false      # <- Skip sampling
analysis:
  enabled: true       # <- Only run analysis
```

Then run with the same command—no additional flags needed!
...
To run multiple experiments with different settings:

1. **Copy the example config:**

   ```bash
   cp fold_pipeline_config.example.yaml my_custom_experiment.yaml
   ```
2. **Modify your config** with experiment-specific settings:

   ```yaml
   # Example modifications
   train_validation_folds_dir: ../data/my_dataset_1000/

   train:
     base_config:
       model:
         mode: lstm              # Try LSTM instead of transformer
         latent_size: 128        # Smaller latent space

   sampling:
     num_unique: 5000            # Generate fewer molecules
     temperature: 0.7            # More deterministic
   ```
3. **Run with your config:**

   ```bash
   python run_fold_pipeline.py --config my_custom_experiment.yaml
   ```

Each config generates its own outputs in `fold_pipeline_outputs/` and `save/fold_pipeline_runs/` based on the `artifacts_output_root` and `training_output_root` settings.
...
To compare multiple sampling runs with different randomness, modify the config seeds:

```yaml
# In fold_pipeline_config.example.yaml, under sampling:
sampling:
  enabled: true
  training_dist_seed: 42          # <- Change this between runs
  # Other sampling settings...

# Or modify the analysis seeds:
analysis:
  overrides:
    # These settings are in analysis_modules/config.py
    random_seed: 42               # <- Per-fold analysis seed
```

Then run the pipeline with your modified config:

```bash
python run_fold_pipeline.py --config my_custom_config.yaml
```
...
| --------------- | ------ | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `--config`    | string | `fold_pipeline_config.example.yaml` | Path to YAML/JSON config file                                                                                              |
| `--only-fold` | int    | `None`                              | Run a**single** CV fold index only (e.g., `0` for fold 0). Useful for debugging. If not specified, runs all folds. |

### Usage Examples:

```bash
# Run all folds with default config
python run_fold_pipeline.py

# Run all folds with custom config
python run_fold_pipeline.py --config my_custom_config.yaml

# Run only fold 0 (faster for quick testing/debugging)
python run_fold_pipeline.py --config fold_pipeline_config.example.yaml --only-fold 0

# Run only fold 2 with custom config
python run_fold_pipeline.py --config my_custom_config.yaml --only-fold 2
```
...
- **Key packages** (see `environment.yml` for full list):
  ...
  - `scikit-learn`: ML utilities (PCA, t-SNE, sampling)
